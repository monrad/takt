...
type SkeletonExtras struct {
    ...
    Decisions   []Decision        // gate answers, waivers, disposition
    Assumptions []spec.Assumption // the spec's own, unfiltered
}
...
- **`ShippedRow{Wave, Slice, Attempt, SHA string, Tasks []int}`** — one row per
  `wave_committed` event, ordered by wave, slice, attempt, including backfilled ones and
  including a retried attempt that also committed. Each is a real commit on the branch;
  hiding one would make the table lie. Task titles come from `plan.Index`. The slice column
  is rendered only when some wave has more than one slice.
- **`Decision{Kind, Subject, Choice, Reason}`** — `Kind` is one of `gate`, `task_waiver`,
  ...
  `internal/finish` stays a function of what it is handed. Sources, in order: every
  `gate_answered` event carrying a non-empty reason; every `task_waived` event; every
  `goal_waived` event; `state.Disposition` (always rendered, reason or not); the
  `user-confirmed` rows of the spec's assumptions table.
...
- It takes no lock and writes no state. Everything it writes is a pure function of the
  bundle, so a concurrent `next` writes identical bytes.
...
- `docs/superpowers/specs/2026-08-24-takt-design.md`: add `finish/retro-skeleton.md` to the
  §4.2 bundle layout; rewrite §7.5 step 3 to describe the skeleton and the seven sections;
  add `retro` to the command list in §6.
...
| Does `takt retro --rewrite` take the run lock? | No | It writes only pure re-derivations, so a concurrent `next` writes identical bytes; taking a lock would let a stale one block a read-only-in-effect command | assumed |
