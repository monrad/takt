You are implementing task 5 of 9 for run sweep-the-plan-4-plan-5-deferred-minors-backlog. Your cwd is the repository root; every path is relative to it.
...
BEGIN UNTRUSTED-ARTIFACT-7a7b49cea0b280cd previous-failure
rework: Implementation is correct, but the required --root regression test is not implemented.
END UNTRUSTED-ARTIFACT-7a7b49cea0b280cd
...
BEGIN UNTRUSTED-ARTIFACT-7a7b49cea0b280cd task-title
hostgen failures share one style and name the path actually read
END UNTRUSTED-ARTIFACT-7a7b49cea0b280cd

BEGIN UNTRUSTED-ARTIFACT-7a7b49cea0b280cd task-description
...
- G9 — hostgen's two adjacent failures share one error style and name the path actually read, which means `RenderCopilotAgent` receives it and its caller in `internal/tools/hostgen` passes it.
...
BEGIN UNTRUSTED-ARTIFACT-7a7b49cea0b280cd spec-excerpt
