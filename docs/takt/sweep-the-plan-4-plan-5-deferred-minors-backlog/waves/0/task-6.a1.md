You are implementing task 6 of 9 for run sweep-the-plan-4-plan-5-deferred-minors-backlog. Your cwd is the repository root; every path is relative to it.
...
BEGIN UNTRUSTED-ARTIFACT-14ecfd8a1a9e4066 task-title
task build stamps the version from the plugin manifest
END UNTRUSTED-ARTIFACT-14ecfd8a1a9e4066

BEGIN UNTRUSTED-ARTIFACT-14ecfd8a1a9e4066 task-description
...
- G10 — `task build` produces a binary whose reported version matches the plugin manifest, while a local `go build`, `go test` and an unstamped `go install ./cmd/takt` still report the dev version. A tagged `go install …@vX.Y.Z` is out of scope: `version.Current` already recovers X.Y.Z from build info.
...
BEGIN UNTRUSTED-ARTIFACT-14ecfd8a1a9e4066 spec-excerpt
