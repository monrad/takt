You are implementing task 1 of 9 for run sweep-the-plan-4-plan-5-deferred-minors-backlog. Your cwd is the repository root; every path is relative to it.
...
BEGIN UNTRUSTED-ARTIFACT-b623581ad0536d82 task-title
Version handshake: refuse an empty --expect, judge once, name the skill
END UNTRUSTED-ARTIFACT-b623581ad0536d82

BEGIN UNTRUSTED-ARTIFACT-b623581ad0536d82 task-description
...
- G1 — `takt version --expect ""` fails the handshake with the versionless refusal instead of exiting 0, `--expect`'s mismatch text names the skill while `--expect-manifest`'s names the plugin, and the handshake judgment is evaluated once per call rather than twice.
...
BEGIN UNTRUSTED-ARTIFACT-b623581ad0536d82 spec-excerpt
