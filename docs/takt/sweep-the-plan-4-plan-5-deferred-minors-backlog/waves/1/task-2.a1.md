You are implementing task 2 of 9 for run sweep-the-plan-4-plan-5-deferred-minors-backlog. Your cwd is the repository root; every path is relative to it.
...
BEGIN UNTRUSTED-ARTIFACT-76fc07d9da0a0aad task-title
Init/next write path: atomic byte writes, gitignore escaping, info/exclude degradation
END UNTRUSTED-ARTIFACT-76fc07d9da0a0aad

BEGIN UNTRUSTED-ARTIFACT-76fc07d9da0a0aad task-description
...
- G2 — All four brief-and-sidecar writes — the stable brief and slice diff in `cmd_next.go`, `renderTaskBrief` in `launch.go`, and `writeLogsIgnore` in `cmd_init.go` — go through one atomic `bundle.WriteFileAtomic`, so no crash can hand an agent a truncated brief.
- G3 — The two `info/exclude` rules are escaped as gitignore patterns, backslash first, so a bundle directory holding a metacharacter or a literal backslash still matches itself.
- G4 — A failure to write `info/exclude` no longer fails `init` or `next`: both report it as a `warnings` entry and exit 0, where `warnings` is an array of strings absent when empty, carried on `op.Op` as an `omitempty` field.
- G11 — The four deferred test gaps are pinned: the lock steal boundary and held-by-self case, the cross-host prompt invariants, and `quotedScalarProblem`'s escape look-back.
...
BEGIN UNTRUSTED-ARTIFACT-76fc07d9da0a0aad spec-excerpt
