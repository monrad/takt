You are implementing task 1 of 4 for run parsereport-in-internal-cli-cmd-record-go-reads. Your cwd is the repository root; every path is relative to it.
...
- G1 — parseReport extracts STATUS, SUMMARY and BLOCKERS from a trailer decorated with leading list, quote or heading markers and with bold, italic or backtick decoration around the key, around the value, or around the whole line.
- G2 — Undecorated trailer lines and the existing digest behaviour are unchanged: exact-prefix lines still parse, the last occurrence still wins, and --status/--summary/--blockers still override.
- G3 — A line that merely mentions STATUS: mid-sentence, a lowercase key, or a key without a colon never records a digest.
- G6 — The change lands clean under the repo's gates.
...
BEGIN UNTRUSTED-ARTIFACT-5db637162ff6aa27 spec-excerpt
