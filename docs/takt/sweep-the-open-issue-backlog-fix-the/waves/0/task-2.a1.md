You are implementing task 2 of 14 for run sweep-the-open-issue-backlog-fix-the. Your cwd is the repository root; every path is relative to it.
...
BEGIN UNTRUSTED-ARTIFACT-4ec2efd498726915 task-title
...
END UNTRUSTED-ARTIFACT-4ec2efd498726915

BEGIN UNTRUSTED-ARTIFACT-4ec2efd498726915 task-description
...
- G2 — `runReview` writes findings, carry, `gate_reviewed` event and receipt in that order and fails on the event write like every other write; the receipt and the event carry the backend's `reason`; on an `error` verdict the `gate_review` question states the reason, says `reviews/<gate>.md` describes the previous pass, and offers `retry` (recommended, re-run the review), `accept` and `stop` but not `revise`; `reviews/<gate>.json` carries `hash` and `round`; and a new `review-record` doctor check WARNs when that hash differs from a non-error, non-skipped receipt's.
- G8 — When merge is blocked, `branch_finish` lists `pr` first labelled "(Recommended)", then `keep`, then the disabled `merge` with its reason, then `discard`; when merge is allowed the order is unchanged; exactly one option is ever recommended and it is enabled.
...
BEGIN UNTRUSTED-ARTIFACT-4ec2efd498726915 spec-excerpt
