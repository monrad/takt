You are implementing task 6 of 14 for run sweep-the-open-issue-backlog-fix-the. Your cwd is the repository root; every path is relative to it.
...
BEGIN UNTRUSTED-ARTIFACT-c3c487fb6d0033b0 previous-failure
failed: verify — go test -race -count=1 ./internal/brief/... ./internal/cli/... exited 1: --- FAIL: TestRecordGoalsReportsALostStreakResetAppend (0.03s)
    cmd_record_test.go:417: the loss must be reported under "warnings": map[problems:[G1: citation "a.go:1" — not a file] valid:false]
FAIL
FAIL	github.com/monrad/takt/internal/cli	6.274s
FAIL
END UNTRUSTED-ARTIFACT-c3c487fb6d0033b0
...
BEGIN UNTRUSTED-ARTIFACT-c3c487fb6d0033b0 task-title
The task brief names the spec by absolute path instead of quoting it; brief-package polish
END UNTRUSTED-ARTIFACT-c3c487fb6d0033b0

BEGIN UNTRUSTED-ARTIFACT-c3c487fb6d0033b0 task-description
...
- G9 — A task brief names the run's `spec.md` by absolute path and tells the implementer to read it as data, and no longer quotes the spec's text: `brief.TaskData` has `SpecPath` and no `SpecExcerpt`, and no rendered brief contains a `spec-excerpt` block.
...
BEGIN UNTRUSTED-ARTIFACT-c3c487fb6d0033b0 spec-excerpt
