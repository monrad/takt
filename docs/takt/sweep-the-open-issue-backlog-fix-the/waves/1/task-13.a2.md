...
BEGIN UNTRUSTED-ARTIFACT-8024a4ac6daf615a previous-failure
rework: Most requested coverage is implemented, but the blind-review leak test weakens an explicit required assertion and can miss leakage of the lens name.
END UNTRUSTED-ARTIFACT-8024a4ac6daf615a
...
BEGIN UNTRUSTED-ARTIFACT-8024a4ac6daf615a task-title
The polish tests, with the fake reviewer recording its calls so the scoped log is read by its exact LogID
END UNTRUSTED-ARTIFACT-8024a4ac6daf615a

BEGIN UNTRUSTED-ARTIFACT-8024a4ac6daf615a task-description
...
BEGIN UNTRUSTED-ARTIFACT-8024a4ac6daf615a spec-excerpt
...
BEGIN UNTRUSTED-ARTIFACT-8024a4ac6daf615a review-findings
major internal/cli/close_internal_test.go:350 — Blind prompt test does not enforce absence of "correctness": The task requires asserting that the exact recorded blind prompt contains neither the claim marker, verifier marker, nor "correctness". This test instead rejects only the narrower string "[lens:correctness]", so leaks such as "Lens: correctness" would pass. Account for the rubric's fixed "correctness and edge cases" occurrence separately, then assert there are no additional or lens-attributing occurrences rather than narrowing the check to one findings.md tag.
END UNTRUSTED-ARTIFACT-8024a4ac6daf615a
