...
BEGIN UNTRUSTED-ARTIFACT-f70d8b2caa2552ea previous-failure
rework: The implementation is largely correct, but it misses the required goals.json diagnostic and generates an invalid or unsafe command for bundle paths requiring shell quoting.
END UNTRUSTED-ARTIFACT-f70d8b2caa2552ea
...
BEGIN UNTRUSTED-ARTIFACT-f70d8b2caa2552ea task-title
...
BEGIN UNTRUSTED-ARTIFACT-f70d8b2caa2552ea task-description
...
BEGIN UNTRUSTED-ARTIFACT-f70d8b2caa2552ea spec-excerpt
...
BEGIN UNTRUSTED-ARTIFACT-f70d8b2caa2552ea review-findings
major internal/cli/finish_test.go:663 — Malformed goals record error does not name goals.json: The task requires the corrupt finish/goals.json case to fail with an error naming goals.json. This assertion only checks for "JSON" because gatherFinishFacts/goalFacts reads the record before preparePushPR can wrap the error with its path, so the actual diagnostic is merely "unexpected end of JSON input". Ensure the earlier error path includes finish/goals.json and assert that filename here.
major internal/brief/templates/run-push_pr.md:4 — PR body path is interpolated as an unquoted shell word: PRBodyPath can be an absolute path under a user-configured or external bundle directory. Spaces split it into multiple arguments, while shell metacharacters can change command interpretation. Shell-quote this generated operand and add coverage using a bundle path containing spaces and an apostrophe or other metacharacter.
