...
- G4 — Answering the capped plan gate works through the existing gate-agnostic paths: *accept* records `gate_overridden` for the plan gate with the reason, *retry* appends `gate_rounds_reset{gate: "plan"}`, *stop* leaves the gate open. · signal: test · evidence: a `cmd_answer` test mirroring the spec case at `cmd_answer_test.go:84`
...
- G6 — The design document no longer contradicts the behaviour: §7.2's "the plan gate keeps today's behaviour entirely, including its uncapped rounds" is gone, §5.3 row 9 carries the cap the way row 6 does, and §5.4 and §5.2 describe the cap as a review gate's rather than the spec gate's. · signal: docs · evidence: the four passages read against the shipped `decidePlan`
