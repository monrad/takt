...
- **Fix wave:** a stale generated holder is taken over without a `lock_taken` event (the event is decided on the holder's record, not the outcome); rejection reasons in retried briefs are quoted with the delimiter token; agent names live in `internal/op` beside the run steps; hostgen treats a generated agent without a source as stale; `--expect` with a blank expectation names the host handshake.
